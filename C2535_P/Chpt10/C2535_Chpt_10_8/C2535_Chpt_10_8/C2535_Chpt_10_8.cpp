...
int main()
{
	...
	{
		month--;
		// Values for month entered by user start at 1 instead of 0
		// Input validation    
		if (month < 0 || month > 11)
		{
...
{    // Look up the standard number of days in the given month
	...
	// Adjust for leap year    if (isLeapYear(year) && month == FEB)
	numberOfDays++;
	// Done    
	return numberOfDays;
}
...
{    // Get current time in seconds since the epoch    
	...
	month = tm_ptr->tm_mon;    year = tm_ptr->tm_year + 1900;
}
/******************************************
*       isLeapYear                       *
******************************************/
